...
- остальные — внешний провайдер (серверный запрос)

| Переменная | Значение | Эффект |
|------------|----------|--------|
| `PLACES_PROVIDER` | `nominatim` (по умолчанию) | OpenStreetMap, без ключей |
| `PLACES_PROVIDER` | `dadata` | DaData — лучше по РФ, нужны ключи |
| `DADATA_API_KEY` | ключ | API-ключ из личного кабинета DaData |
| `DADATA_SECRET_KEY` | секрет | Секретный ключ из личного кабинета DaData |

Ключи DaData: [dadata.ru/profile](https://dadata.ru/profile/) → раздел «API».
